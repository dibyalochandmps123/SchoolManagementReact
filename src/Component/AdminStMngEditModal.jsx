...
import {putEditExistsClass} from "../API/classManagement";
export default function AdminStMngEditModal({ showEdit, handleCloseEdit, selectedEditData, handleUpdateEditData,subjectList }){
  ...
  const handleEditSubmit = () => {
    ...
    formEditData.timetable.forEach((row) => {
      const dayKey = row[0].toLowerCase();
      formattedTimetable[dayKey] = row.slice(1);
    });
    const payload = {
      standard: formEditData.class,
      section: formEditData.section,
      totalStudents: formEditData.seats,
      classTeacher: formEditData.teacher,
      roomNumber: formEditData.room,
      timeTable: formattedTimetable
    };
    // Call backend function with payload
    putEditExistsClass(payload);
    handleCloseEdit();
};
